...
_share_client: Optional[ShareClient] = None
...
def _get_backend() -> StorageBackend:
    ...
    if Setting.get("use_azure_file").get_value():
        return StorageBackend.AZURE_FILE
...
async def _get_share_client() -> ShareClient:
    """Return a cached Azure ShareClient instance."""
    global _share_client
    if _share_client is None:
        conn = Setting.get("azure_file_connection_string").get_value()
        share = Setting.get("azure_file_share").get_value()
        _share_client = ShareClient.from_connection_string(conn, share_name=share)
...
async def upload_bytes(data: bytes, path: str) -> None:
    ...
    elif backend is StorageBackend.AZURE_FILE:
        client = await _get_share_client()
        file = client.get_file_client(path)
